...
import { createLessonStepSchema, updateLessonStepSchema, } from "../schemas/lessonStepSchemas.js";
...
export const updateLessonStep = async (req, res) => {
    try {
        const userId = req.user?.id;
        const lessonStepId = typeof req.params.id === "string" ? req.params.id : undefined;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
            return;
        }
...
export const getLessonStepsByPlan = async (req, res) => {
    try {
        const userId = req.user?.id;
