...
import { generateAiLessonRequestSchema, } from "../schemas/aiLessonSchemas.js";
...
import { ensureSufficientAiCredits, consumeAiCredits, } from "../services/aiUsageService.js";
export const generateAiLesson = async (req, res) => {
    try {
...
export const generateAndSaveAiLesson = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({ error: "Unauthorized" });
        ...
        const lessonPlan = await prisma.lessonPlan.create({
            data: {
                ...
                dependencies: draft.lessonPlan.dependencies,
            },
        });
        const savedSteps = [];
        for (const step of draft.lessonSteps) {
            const savedStep = await prisma.lessonStep.create({
                data: {
                    ...
                    snippetExports: step.snippetExports,
                },
            });
