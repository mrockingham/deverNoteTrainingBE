...
export const getPracticeSessions = async (
  ...
  try {
    ...
    const lessonPlanId =
      typeof req.query.lessonPlanId === "string"
...
const submittedStepIndex = lessonSteps.findIndex(
  (step) => step.id === data.lessonStepId,
);
...
export const resetLessonPractice = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
